...
function RefreshYields()
	...
	if GameCapabilities.HasCapability("CAPABILITY_SCIENCE") and GameCapabilities.HasCapability("CAPABILITY_DISPLAY_TOP_PANEL_YIELDS") then
		...
		local fakeTechId			:number = GameInfo.Technologies['TECH_FAKE_TECH'].Index;
		local overflow				:number = playerTechnology:GetResearchProgress(fakeTechId);
		m_ScienceYieldButton.YieldBalance:SetText( FormatValuePerTurn(currentScienceYield) );
		local ttp					:string = GetScienceTooltip();
		if overflow > 0 then
			m_ScienceYieldButton.YieldPerTurn:SetText( Locale.ToNumber(overflow, "#,###.#") .. Locale.Lookup('LOC_OVERFLOW_NAME') );
			m_ScienceYieldButton.YieldBacking:SetToolTipString( ttp .. '[NEWLINE][NEWLINE]' .. Locale.Lookup('LOC_CLICK_TO_APPLY_OVERFLOW_TO_CURRENT_TECH') );
		else
			m_ScienceYieldButton.YieldPerTurn:SetText( '' );
			m_ScienceYieldButton.YieldBacking:SetToolTipString( ttp .. '[NEWLINE][NEWLINE]' .. Locale.Lookup('LOC_CLICK_TO_SWITCH_TO_OVERFLOW_TECH') );
		end
		m_ScienceYieldButton.YieldIconString:SetText("[ICON_ScienceLarge]");
		m_ScienceYieldButton.YieldButtonStack:CalculateSize();
		m_ScienceYieldButton.YieldBacking:RegisterCallback(Mouse.eLClick, OnScienceYieldButtonClicked);
	end	
	...
	if GameCapabilities.HasCapability("CAPABILITY_CULTURE") and GameCapabilities.HasCapability("CAPABILITY_DISPLAY_TOP_PANEL_YIELDS") then
		m_CultureYieldButton = m_CultureYieldButton or m_YieldButtonDoubleManager:GetInstance();
		local playerCulture			:table	= localPlayer:GetCulture();
		local currentCultureYield	:number = playerCulture:GetCultureYield();
		local fakeCivicId			:number = GameInfo.Civics['CIVIC_FAKE_CIVIC'].Index;
		local overflow				:number = playerCulture:GetCulturalProgress(fakeCivicId);
		local ttp					:string = GetCultureTooltip();
		m_CultureYieldButton.YieldBalance:SetText( FormatValuePerTurn(currentCultureYield) );
		if overflow > 0 then
			m_CultureYieldButton.YieldPerTurn:SetText( Locale.ToNumber(overflow, "#,###.#") .. Locale.Lookup('LOC_OVERFLOW_NAME') );
			m_CultureYieldButton.YieldBacking:SetToolTipString( ttp .. '[NEWLINE][NEWLINE]' .. Locale.Lookup('LOC_CLICK_TO_APPLY_OVERFLOW_TO_CURRENT_CIVIC') );
		else
			m_CultureYieldButton.YieldPerTurn:SetText( '' );
			m_CultureYieldButton.YieldBacking:SetToolTipString( ttp .. '[NEWLINE][NEWLINE]' .. Locale.Lookup('LOC_CLICK_TO_SWITCH_TO_OVERFLOW_CIVIC') );
		end
		m_CultureYieldButton.YieldBalance:SetColorByName("ResCultureLabelCS");
		...
		m_CultureYieldButton.YieldBacking:RegisterCallback(Mouse.eLClick, OnCivicYieldButtonClicked);
	end

	-- xiaoxiao: end editing code
